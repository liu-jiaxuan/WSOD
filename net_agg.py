...
class SCWSSOD(nn.Module):
    def __init__(self, cfg):
        ...
        self.linear2_bbox=nn.Conv2d(256, 1, kernel_size=3, stride=1, padding=1)
        self.initialize()
    ...
    def forward(self, x, mode=None):
        ...
        out2_init = self.srm2(self.fam23(out2, out3, out2_a))
        ...
        if mode == 'Test':
            ...
            out2 = F.interpolate(self.linear2(out2_init), size=x.size()[2:], mode='bilinear', align_corners=False)
            out2_bbox=F.interpolate(self.linear2_bbox(out2_init), size=x.size()[2:], mode='bilinear', align_corners=False)
            return out2,out2_bbox, out3, out4, out5
        elif mode== 'Train':
            # ------------------------------------------------------ TRAIN ----------------------------------------------------
            ...
            out2  = torch.sigmoid(F.interpolate(self.linear2(out2_init), size=x.size()[2:], mode='bilinear', align_corners=False))
            out2_bbox=torch.sigmoid(F.interpolate(self.linear2_bbox(out2_init), size=x.size()[2:], mode='bilinear', align_corners=False))
            ...
            return out2,out2_bbox, out3, out4, out5,out2_init
        elif mode=='vis':
            out5 = F.interpolate(self.linear5(out5), size=x.size()[2:], mode='bilinear', align_corners=False)
            out4 = F.interpolate(self.linear4(out4), size=x.size()[2:], mode='bilinear', align_corners=False)
            out3 = F.interpolate(self.linear3(out3), size=x.size()[2:], mode='bilinear', align_corners=False)
            out2 = F.interpolate(self.linear2(out2_init), size=x.size()[2:], mode='bilinear', align_corners=False)
            out2_bbox = F.interpolate(self.linear2_bbox(out2_init), size=x.size()[2:], mode='bilinear',
                                      align_corners=False)
            return out2, out2_bbox, out3, out4, out5,out2_init
    def initialize(self):
        weight_init(self)